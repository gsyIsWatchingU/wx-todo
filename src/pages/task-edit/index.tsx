...
export default function TaskEditPage({}: TaskEditPageProps) {
  ...
  useEffect(() => {
    const { id, date } = Taro.getCurrentInstance().router?.params || {}
  ...
  const loadTask = async (id: string) => {
    try {
      ...
      if (!task || isUnmountedRef.current) {
        return
      }
  ...
  const handleDelete = async () => {
    if (!isEditMode || !taskId || isUnmountedRef.current) {
      return
    }
    ...
    Taro.showModal({
      ...
      content: '确定要删除这个任务吗？',
      success: async (res) => {
        if (!res.confirm || isUnmountedRef.current) {
          return
        }
  ...
  return (
    <View className='task-edit-page'>
      <View className='header'>
        <Text className='cancel-btn' onClick={() => Taro.navigateBack()}>
          取消
        </Text>
      ...
      <View className='form'>
        ...
        <View className='form-section'>
          <View className='row'>
            <Text className='section-label'>设置截止时间</Text>
            <View
          ...
          {hasDueDate && (
            <View className='date-time-pickers'>
              <Picker mode='date' value={dueDate} onChange={onDateChange}>
                <View className={`picker ${dueDate ? 'selected' : ''}`}>
                  <Text>{dueDate || '选择截止日期'}</Text>
                </View>
              </Picker>
              <Picker mode='time' value={dueTime} onChange={onTimeChange}>
                <View className={`picker ${dueTime ? 'selected' : ''}`}>
                  <Text>{dueTime || '选择截止时间'}</Text>
                </View>
              </Picker>
            </View>
          )}
